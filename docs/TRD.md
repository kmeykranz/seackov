...
- `RunStorySystem` owns run-local story objective markers, minimap target data, `E` hold interactions, objective HUD updates, tunnel trigger detection, and stage-completion visual effects.
...
- `ProgressState` is a Godot autoload that owns persistent story stage, region unlock state, uploaded legendary count, signal/tunnel completion records, final ending state, pending/uploaded knowledge, unlocked tools, and debug save mutation APIs.
- `BoatScene` owns the boat interaction zones, feeds task/spawn data into the mission panel, writes selected spawn anchors to `ProgressState`, and delegates item state changes to `PlayerInventory`.
- `MissionConsoleUi` owns the centered boat task panel, left-side task rows, right-side spawn anchor buttons, and selected-anchor click signals.
...
- `RunLevelBuilder` instantiates separate scenes into the run scene containers, generates segmented perimeter walls, configures player soft world bounds, builds coral and seaweed cover, and selects a spawn anchor from the deepest unlocked region unless a saved selected anchor is valid.
...
- `scenes/ui/mission_console_ui.tscn`
...
- `scripts/story/run_story_system.gd`
...
- `Node._unhandled_input` and `InputEventKey` for `B` inventory toggles in run and boat scenes, `M` minimap toggles, `Esc` pause toggles in the run scene, and one-second `E` terminal dismissal holds.
...
- `Node._process` timers for story objective hold progress, marker pulsing, and temporary clearing-beam effects.
- Project autoloads for runtime cross-scene inventory state.
- Project autoloads for persistent story and region progression state.
...
- `Node.create_tween()` for terminal overlay fadeout after hold-to-dismiss confirmation.

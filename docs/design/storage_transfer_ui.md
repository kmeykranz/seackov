...
## Problem Definition
The boat scene has runtime backpack and warehouse counts, and the run scene needs a backpack-only view of the same items. Storage movement needs to feel like a grid inventory: items should appear as icons in slots, and common Minecraft-style clicks should move stacks without opening extra dialogs.
...
- Add a small fixed grid UI with hand-held stack state and a backpack-only mode: selected because it supports the requested click behavior while keeping save data, equipment, and crafting out of scope.
...
- `OpenBackpackOnly`: only backpack slots are visible in the run scene.
...
- `press_b_in_run`: toggles between `Closed` and `OpenBackpackOnly`.
...
- Shift-click is disabled in backpack-only mode because no warehouse target is visible.
...
- Backpack-only mode hides warehouse slots and shrinks the panel to the backpack column.
...
Rollback Path
Remove `scenes/ui/storage_transfer_ui.tscn`, `scripts/ui/storage_transfer_ui.gd`, `scripts/ui/storage_slot.gd`, the slot-level `PlayerInventory` methods, and the `StorageTransferUi` instances from `boat_scene.tscn` and `run_scene.tscn`. Restore the previous storage transfer UI behavior.

## Primitive Acceptance Criteria
- Pressing `B` in the boat scene opens the grid storage UI.
- Pressing `B` in the run scene opens a backpack-only grid storage UI.
...
- Run-scene storage UI does not show warehouse slots.
